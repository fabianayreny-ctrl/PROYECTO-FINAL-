...
def cambiar_contrasena_admin():
    nueva_contrasena = "admin12345"

    salt, hash_ = generar_hash_contrasena(nueva_contrasena)

    with obtener_conexion() as conexion:
        conexion.execute(
            """
            UPDATE usuarios
            SET contrasena_hash = ?, contrasena_salt = ?
...
def historial_paciente(paciente_id: int, usuario=Depends(obtener_usuario_actual)):
